...
function createDatabaseSchema(callback) {
  ...
  db.run(
    `CREATE TABLE movies
    (movie_id INTEGER PRIMARY KEY, title TEXT, year INTEGER, released TEXT,
  ...
  db.run("CREATE TABLE genre(genre_id INTEGER PRIMARY KEY, name TEXT)");
  db.run(
    "CREATE TABLE actors(actors_id INTEGER PRIMARY KEY, first_name TEXT, last_name TEXT)"
  );
  callback();
}
...
class DatabaseImporter {
  ...
  importMovie(movie) {
    ...
    db.run(`INSERT INTO genre ${movies.genre}`);
  }
}

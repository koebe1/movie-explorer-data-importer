...
function createDatabaseSchema(callback) {
  ...
  db.run(
    "CREATE TABLE genres (genre_id INTEGER PRIMARY KEY, name TEXT, UNIQUE(name))"
  );

  db.run(
    "CREATE TABLE actors(actors_id INTEGER PRIMARY KEY AUTOINCREMENT, first_name TEXT, last_name TEXT)"
  );
